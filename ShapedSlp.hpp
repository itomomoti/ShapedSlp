...
private:
  ...
  sdsl::bit_vector stgDiv_;
  sdsl::select_support_mcl<> stgDivSel_;
  sdsl::bit_vector slpDiv_;
  sdsl::select_support_mcl<> slpDivSel_;
  DacT vlcSeq_;
  DacT vlc_;
...
public:
  ...
  (
   const bool verbose = false
    ...
    const size_t bytesStgDiv = sdsl::size_in_bytes(stgDiv_);
    const size_t bytesStgDivSel = sdsl::size_in_bytes(stgDivSel_);
    const size_t bytesSlpDiv = sdsl::size_in_bytes(slpDiv_);
    const size_t bytesSlpDivSel = sdsl::size_in_bytes(slpDivSel_);

    const size_t bytesMph = calcMemBytesOfMph();
    const size_t bytesTotal = bytesMph + bytesSeqSBV + bytesVlcSeq + bytesVlc + bytesStgDiv + bytesStgDivSel + bytesSlpDiv + bytesSlpDivSel + bytesAlphSize;
    ...
    std::cout << "| stgDiv = " << bytesStgDiv + bytesStgDivSel << std::endl;
    std::cout << "| | bitv = " << bytesStgDiv << std::endl;
    std::cout << "| | sele = " << bytesStgDivSel << std::endl;
    std::cout << "| slpDiv = " << bytesSlpDiv + bytesSlpDivSel << std::endl;
    std::cout << "| | bitv = " << bytesSlpDiv << std::endl;
    std::cout << "| | sele = " << bytesSlpDivSel << std::endl;
  ...
  (
   std::istream & in
    ...
    stgDivSel_.set_vector(&stgDiv_);
    slpDivSel_.set_vector(&slpDiv_);
  ...
  (
   std::ostream & out
    ...
    stgDiv_.serialize(out);
    stgDivSel_.serialize(out);
    slpDiv_.serialize(out);
   ...
   const bool freqSort = true
    ...
    std::vector<var_t> stgOffset(stg.getNumRules());
    {
      ...
      if (freqSort) {
        ...
        std::vector<uint64_t> alphFreq(stg.getNumRules());
      ...
      stgDiv_ = sdsl::bit_vector(stg.getNumRules(), 0);
      uint64_t offset = 0;
      ...
      stgDiv_[stg.getNumRules() - 1] = 1;
      stgOffset[stgOrder[stg.getNumRules() - 1]] = offset;
      stgDivSel_.init_slow(&stgDiv_);
    }
    ...
    std::vector<var_t> slpOffset(slp.getNumRules());
    {
      ...
      if (freqSort) {
        ...
        std::vector<uint64_t> alphFreq(slp.getNumRules()); // TODO: sorting alphabet based on frequency
      ...
      slpDiv_ = sdsl::bit_vector(slp.getNumRules(), 0);
      uint64_t offset = 0;
      ...
      slpDiv_[slp.getNumRules() - 1] = 1;
      slpOffset[slpOrder[slp.getNumRules() - 1]] = offset;
      slpDivSel_.init_slow(&slpDiv_);
    }
