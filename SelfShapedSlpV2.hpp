#ifndef INCLUDE_GUARD_SelfShapedSlpV2
#define INCLUDE_GUARD_SelfShapedSlpV2
...
// #define PRINT_STATUS_SelfShapedSlpV2
...
private:
  static constexpr uint8_t kVipPrefLen = 64;
  static constexpr uint8_t kVipNumW = 6;
  static constexpr uint8_t kVipThres = 2;
  ...
  sdsl::int_vector<kVipNumW> vipNum_;
  DacT vlcSeq_;
  DacT vlc_;
  BalDacT bal_;
...
public:
  ...
  (
   ...
   const var_t slpOffset //!< slp offset for the variable 
    ...
    const uint64_t slpIdBase = slpDivSel_(h + 1);
    const uint64_t slpId = slpIdBase + slpOffset;
    const uint64_t balPos = (slpOffset < vipNum_[h] * kVipPrefLen)? h + balBvRank_(slpIdBase - h) + (slpOffset / kVipPrefLen) : h + balBvRank_(slpId - h);
  ...
  (
   const bool verbose = false
    ...
    std::cout << "SelfShapedSlpV2 object (" << this << ") " << __func__ << "(" << verbose << ") BEGIN" << std::endl;
    ...
    std::cout << "alphSize = " << alphSize << ", len = " << len << ", lenSeq = " << lenSeq
              ...
              << std::endl;
    std::cout << "kVipPrefLen = " << (int)kVipPrefLen << ", kVipNumW = " << (int)kVipNumW << ", kVipThres = " << (int)kVipThres << std::endl;
    ...
    const size_t bytesVipNum = sdsl::size_in_bytes(vipNum_);

    const size_t bytesMph = calcMemBytesOfMph();
    const size_t bytesTotal = bytesAlphSize + bytesMph + bytesSeqSBV + bytesVlcSeq + bytesVlc + bytesSlpDivSel + bytesBal + bytesBalBv + bytesBalBvRank + bytesVipNum;
    ...
    std::cout << "| vipNum = " << bytesVipNum << std::endl;
    ...
    std::cout << "SelfShapedSlpV2 object (" << this << ") " << __func__ << "(" << verbose << ") END" << std::endl;
  }
  ...
  (
   ...
   const bool freqSort = true
    ...
    std::vector<var_t> slpOffset(slp.getNumRules());
    {
      ...
      // vipNum_.width(kVipNumW);
      vipNum_.resize(rs_->size());
      for (uint64_t i = 0; i < vipNum_.size(); ++i) {
        vipNum_[i] = 0;
      }
      { // calc vip
        const uint64_t maxVipNum = (1ULL << kVipNumW) - 1;
        std::vector<uint64_t> ids;
        std::vector<uint64_t> begs;
        uint64_t soBeg = 0;
        uint64_t prefLen = 0;
        uint64_t vipNumPos = 0;
        uint64_t vipNum = 0;
        bool stillVip = true;
        uint64_t sumFreq = 0;
        uint64_t sumFreqThres = 0;
        begs.push_back(0);
        for (uint64_t i = 0; i < slp.getNumRules(); ++i) {
          const uint64_t id = slpOrder[i];
          ids.push_back(id);
          ++prefLen;
          if (stillVip) {
            if (prefLen <= kVipPrefLen) {
              sumFreq += ruleFreq[id];
            } else if (prefLen == kVipPrefLen + 1) {
              stillVip = (sumFreq >= sumFreqThres);
              if (vipNum == 0) {
                sumFreqThres = sumFreq / kVipThres;
              }
              vipNum += stillVip;
              if (vipNum == maxVipNum) {
                stillVip = false;
          ...
          if (i < slp.getNumRules() - 1 and hashVal[id] == hashVal[slpOrder[i+1]]) {
            if (slp.getLenOfVar(slp.getLeft(id), slplen) != slp.getLenOfVar(slp.getLeft(slpOrder[i+1]), slplen)) {
              if (stillVip and prefLen < kVipPrefLen + 1) {
                stillVip = false;
              }
              prefLen = 0;
              begs.push_back(i + 1 - soBeg);
            }
            continue;
          }

          begs.push_back(i + 1 - soBeg);
          vipNum_[vipNumPos++] = vipNum;
          uint64_t soPos = soBeg;
          for (uint64_t i = 0; i < vipNum; ++i) {
            const uint64_t b = begs[i];
            for (uint64_t j = 0; j < kVipPrefLen; ++j) {
              slpOrder[soPos++] = ids[b + j];
            }
          }
          for (uint64_t i = 0; i < begs.size() - 1; ++i) {
            for (uint64_t j = (i < vipNum) ? begs[i] + kVipPrefLen : begs[i]; j < begs[i+1]; ++j) {
              slpOrder[soPos++] = ids[j];
            }
          }
          ids.clear();
          begs.clear();
          stillVip = true;
          prefLen = sumFreq = sumFreqThres = vipNum = 0;
          soBeg = i + 1;
          begs.push_back(0);
        }
      }
      ...
      uint64_t vipNumPos = 0;
      for (uint64_t i = 0; i < slp.getNumRules(); ++i) {
        if (i == 0 or hashVal[slpOrder[i-1]] != hashVal[slpOrder[i]]) {
          const uint64_t vipNum = vipNum_[vipNumPos++];
          for (uint64_t j = 0; j < vipNum * kVipPrefLen; ++j) { // padding
            balBv_[numZeros++] = 0;
          }
          i += vipNum * kVipPrefLen;
          continue;
        }
...
#ifdef PRINT_STATUS_SelfShapedSlpV2
      {
        std::cout << "width = " << (uint64_t)vipNum_.width() << std::endl;
      ...
      }
      {
        ...
        for (uint64_t i = 0; i < slpOrder.size(); ++i) {
          ...
          ofs << slplen[slpId] << "," << slp.getLenOfVar(slp.getLeft(slpId), slplen) << "," << ruleFreq[slpId] << "," << slpOffset[slpId] << "," << slpId << std::endl;
        }
      }      
#endif
    }
